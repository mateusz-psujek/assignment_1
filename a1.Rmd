...
#  - make a plot of the real parameter values and model estimates
...
#write_rds(models_data, 'models_data.rds')

models_data <- read_rds('models_data.Rds')
...
est <- rbind(f_m %>% gather_draws(`b_.*`, regex = T) %>% mean_qi,
             f_m %>% gather_draws(`sd_.*`, regex = T) %>% mean_qi) %>% 
  select(!c(.variable, .point, .interval)) %>% 
  rename_with(.cols = everything(), ~ paste0("est_", .x))
est <-rbind(est, est %>% slice(c(5,6)))


t_value <- p %>% ungroup %>% filter(!parameter == 'error')
t_value <- rbind(t_value %>% select(mu_log) %>% rename('t_value' = mu_log),
                 t_value %>% select(sd_log) %>% rename('t_value' = sd_log))
...
t_vs_est <- tibble(term = 
                   ...
                   t_value,
                   est) %>% 
  rename_with(.cols = everything(), .fn = ~ str_remove_all(.x, fixed('.')))
...
#### hypotheses testing
```{r}
hypothesis(f_m, `b_diagnosistd:visit` > `b_diagnosisasd:visit`)
```
#### individual effects
```{r}
