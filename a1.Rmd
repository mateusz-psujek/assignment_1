...
utterance_data <- read_csv('lu_train.csv')
words_data <- read_csv('token_train.csv')
```
...
demo_data <- demo_data %>% 
  ...
  mutate(id = as_factor(id), Visit = as.numeric(visit))

utterance_data <- utterance_data%>% 
  rename(id = SUBJ, visit = VISIT) %>% 
  mutate(visit = str_remove_all(visit, '\\D*'), visit = as.numeric(visit))
...
```{r}
#did it as part of the previous point without reading this one, but less efficiently :ccc (str_remove_all instead of str_extract)
...
```{r}
x <- "* id, * visit, * Diagnosis, * Ethnicity, * Gender, * Age, * ADOS,  * MullenRaw, * ExpressiveLangRaw, * Socialization, * MOT_MLU, * CHI_MLU, * types_MOT, * types_CHI, * tokens_MOT, * tokens_CHI." %>% 
  str_extract_all('\\w+') %>% 
  unlist()

data <- merge(demo_data, utterance_data, all = T) %>% 
  merge(words_data, all = T)
...
```{r}
#i did it in the previous section thinking it would make selecting easier
...
visit1_df <- data %>% 
  ...
  group_by(id) %>% 
  mutate(across(.cols = 2:5, .fns = ~ replace(x = .x, values = .x[visit == 1]))) %>% 
  rename_with(.fn = ~ paste0(., "_1"), .cols = 3:6 )
...
```{r}
final_df <- left_join(data, visit1_df) %>% 
  rename_with(~ tolower(.x)) %>% 
  group_by(id) %>%
  mutate(gender = if_else(gender == 1, 'male', 'female'),
         diagnosis = if_else(diagnosis == 'A', 'ASD', 'TD'),
         id = paste0(c(sample(LETTERS, 1), sample(letters, 4)), collapse = ''))


write_csv(final_df, 'cleaned_data.csv')
```
