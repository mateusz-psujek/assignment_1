...
priors <- c(
  ...
  prior(normal(0.2, 0.15), class = b, coef = diagnosisasd),
  prior(normal(0.2, 0.15), class = b, coef = diagnosistd),
  prior(normal(0, 0.35), class = sd, coef = Intercept, group = id),
...
```{r}
df <- left_join(spread_draws(fg_m, `.*b_.*`, regex = TRUE),
             spread_draws(fg_m, `.*sd_.*`, regex = TRUE)) %>% 
  select(-any_of(c("sd_id__Intercept:diagnosistd", "sd_id__visit:diagnosistd")))
  select(-any_of(c(`sd_id__Intercept:diagnosistd`, `sd_id__visit:diagnosistd`)))
...
sds <-  seq(0.1, 1.5, by = 0.1)
sds <- c(0.1, 0.9) #just to check it, REMOVE later
...
models_data <- models_data %>% mutate(sds = sds)
