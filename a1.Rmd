...
# you have to change this i guess (look Riccardo's feedback)
...
formulas <- list(m_env, m_cog, m_env_cog)
```
...
###### Model comparison
```{r}
models <- models[1] # to_do!!! remove this one after trying it
```
...
#cross-validation performance
  ...
  ggplot(aes(x = type, y = rmse, colour = model)) +
    geom_point() +
    theme_minimal() +
    ggtitle("Cross-validation, k = 5")



loo_compare(kfs, criterion = 'kfold')
...
training_rmses <- preds %>% map(~ rmse(y = data$mlu, yrep = .x))

row_bind(
  training_rmses %>% mutate(type = 'training'),
  kfs_rmses %>% mutate(type = 'cross validation')
) %>% 
  ggplot(aes(x = type, y = rmse, colour = model)) +
    geom_point() +
    geom_line() +
