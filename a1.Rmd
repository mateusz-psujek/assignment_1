...
library('tidyverse', 'brms', 'rstan', 'conflicted')
...
power_data <- tibble(N = c(10,100, by = 10)) %>% 
   rowwise %>% 
   mutate(seed = list(seq(1,100, by = 1))) %>% 
   unnest(seed) %>% 
   mutate(dfs = map2(.x = seed, .y = N, .f = ~ update_fg(.x, .y))) %>% 
   unnest(dfs)

  
map(.x = power_data %>% group_split(n) %>% filter(variable == 'diff'),
    .f = ~
