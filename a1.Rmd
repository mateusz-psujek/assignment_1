...
pacman::p_load(tidyverse, brms, ids, rstan, tidybayes, conflicted)
...
#to_do list:
  ...
  # - transform the p tibble so that it makes sense parameter = c(intercept, beta, sigma), mean, sd. Then it is easy to do the next point
  # - you can try to make the mutate in the p a general function instead of doing it separately for each 
          #map2 maybe!!!     or this          p[, grep("intercept_mu", colnames(p)) + 1]
...
priors <- c(
  ...
  prior(normal(0.40, 0.1), class = b, coef = diagnosisasd),
  prior(normal(0.40, 0.1), class = b, coef = diagnosistd),
...
pmap(.l = list(.x = .gg_priors, .y = .gg_posteriors, .t = .gg_titles),
     .f = function(.x, .y, .t){ggplot()+
            geom_density(aes(.x, fill = 'steelblue'))+
            geom_density(aes(.y, fill = '#FC4E07'))+
            ggtitle(.t)}
)
}

pp_update_plot(.fit = f_m, .priors = priors)
```
### Model exploration
```{r}
f_m_s2 <- f_m %>% spread_draws(b_diagnosisasd)
```
### Model comparison
```{r}
