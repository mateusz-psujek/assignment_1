...
pacman::p_load(tidyverse, brms, ids, rstan, tidybayes, conflicted, broom.mixed, gridExtra)
...
posteriors <- select(df, !starts_with('prior')) %>% as.list
priors <- select(df, starts_with('prior')) %>% as.list
titles <- names(posteriors)

pmap(.l = list(.x = priors, .y = posteriors, .t = titles),
     .f = function(.x, .y, .t){
       ggplot()+
...
write_rds(models_data, paste0(deparse(substitute(formula)), "_", "sensitivity", ".Rds"))
...
```{r}
# comparing model estimates with the true generative process
...
#creating a function that creates n_sim simulated dataframes along the function specified in the first part (here: simulate_df(n) - sidenote: n in thisfunction is the total number of participants) and then updates the model fit (here: MLU_m1) accordingly. iter in update is lowered to 1000 to complete the process faster
#s3 saves the model specifications, in my case, the parameters are DiagnosisASD, DiagnosisTD, DiagnosisASD:Visit, DiagnosisTD:Visit - sidenote: use fixef() to see your model's parameters
...
s3 <-
  tibble(seed = 1:n_sim) %>% 
  mutate(b1 = map(seed, sim_and_fit, n = 40)) %>% 
  unnest(b1)
view(s3)
#think i still need to transform the model specifications from logscale with exp()
```
```{r}
#filtering by parameter to get the effects for each parameter 
DiaASD_df <- s3 %>% 
 filter(parameter == "DiagnosisASD") %>% 
  rename_with( ~paste(.x, "DiaASD", sep = "_"), .cols = c("Estimate", "Est.Error", "Q2.5", "Q97.5")) %>% 
  mutate(parameter = NULL)

DiaTD_df <- s3 %>% 
 filter(parameter == "DiagnosisTD") %>% 
  rename_with( ~paste(.x, "DiaTD", sep = "_"), .cols = c("Estimate", "Est.Error", "Q2.5", "Q97.5")) %>% 
  mutate(parameter = NULL)

DiaASD_V_df <- s3 %>% 
 filter(parameter == "DiagnosisASD:Visit") %>% 
  rename_with( ~paste(.x, "DiaASD_V", sep = "_"), .cols = c("Estimate", "Est.Error", "Q2.5", "Q97.5"))%>%
  mutate(parameter = NULL)

DiaTD_V_df <- s3 %>% 
 filter(parameter == "DiagnosisTD:Visit") %>% 
  rename_with( ~paste(.x, "DiaTD_V", sep = "_"), .cols = c("Estimate", "Est.Error", "Q2.5", "Q97.5"))%>%
  mutate(parameter = NULL)

powerdata <- join_all(list(DiaASD_df, DiaTD_df, DiaASD_V_df, DiaTD_V_df), by = "seed", type = "left")%>%
  mutate(diffInterceptEst = Estimate_DiaTD - Estimate_DiaASD) %>% 
  mutate(diffInterceptQ2.5 = Q2.5_DiaTD - Q2.5_DiaASD) %>% 
  mutate(diffInterceptQ97.5 = Q97.5_DiaTD - Q97.5_DiaASD) %>% 
  mutate(diffSlopeEst = Estimate_DiaTD_V - Estimate_DiaASD_V) %>%
  mutate(diffSlopeQ2.5 = Q2.5_DiaTD_V - Q2.5_DiaASD_V) %>% 
  mutate(diffSlopeQ97.5 = Q97.5_DiaTD_V - Q97.5_DiaASD_V) %>%
  mutate(formulaEst = diffInterceptEst + 2.5 * diffSlopeEst) %>% 
  mutate(formulaQ2.5 = diffInterceptQ2.5 + 2.5 * diffSlopeQ2.5) %>%
  mutate(formulaQ97.5 = diffInterceptQ97.5 + 2.5 * diffSlopeQ97.5)
#plotting for intercept - the plots show the span of model parameters for each set of simulated data
 powerdata %>% 
 ggplot(aes(x = reorder(seed, diffInterceptQ2.5), y = diffInterceptEst , ymin = diffInterceptQ2.5, ymax = diffInterceptQ97.5)) +
  geom_hline(yintercept = c(0, .5), color = "white") +
  geom_pointrange(fatten = 1/2) +
  scale_x_discrete("for intercepts", breaks = NULL) +
  ylab(expression(beta[1])) +
  coord_cartesian(ylim = c(-.5, 1.3))
#power analysis for intercept
 powerdata %>% 
  mutate(checkInt = ifelse(diffInterceptQ2.5 > 0, 1, 0)) %>% 
  summarise(powerInt = mean(checkInt))
#plot for slope
 powerdata %>% 
 ggplot(aes(x = reorder(seed, diffSlopeQ2.5), y = diffSlopeEst , ymin = diffSlopeQ2.5, ymax = diffSlopeQ97.5)) +
  geom_hline(yintercept = c(0, .5), color = "white") +
  geom_pointrange(fatten = 1/2) +
  scale_x_discrete("for slopes", breaks = NULL) +
  ylab(expression(beta[1])) +
  coord_cartesian(ylim = c(-.5, 1.3))
#power analysis for slope
powerdata %>% 
  mutate(checkSlo = ifelse(diffSlopeQ2.5 > 0, 1, 0)) %>% 
  summarise(powerSlo = mean(checkSlo))
 #plot for formula - the formula approach is a bit janky, but since we dont just have random intercepts, but also slopes, we need to improvise: the formula is difference of intercepts + difference of slopes * 2.5(mean of Visit) -> this gives us the mean difference between the diagnoses
  powerdata %>% 
 ggplot(aes(x = reorder(seed, formulaQ2.5), y = formulaEst , ymin = formulaQ2.5, ymax = formulaQ97.5)) +
  geom_hline(yintercept = c(0, .5), color = "white") +
  geom_pointrange(fatten = 1/2) +
  scale_x_discrete("for formula", breaks = NULL) +
  ylab(expression(beta[1])) +
  coord_cartesian(ylim = c(-.5, 1.3))
 #power analysis for formula
  powerdata %>% 
  mutate(checkformula = ifelse(formulaQ2.5 > 0, 1, 0)) %>% 
  summarise(powerformula = mean(checkformula))
```
```{r}
#precision time - precision is calculated by the width of the Confidence Intervals
#intercept
powerdata <-powerdata%>% 
  mutate(widthInt = abs(diffInterceptQ97.5 - diffInterceptQ2.5))

powerdata%>% 
  mutate(checkInt = ifelse(widthInt < .7, 1, 0)) %>% 
  summarise(`proportion below 0.7` = mean(checkInt),
            `average width`        = mean(widthInt))
#slope
powerdata <-powerdata%>% 
  mutate(widthSlo = abs(diffSlopeQ97.5 - diffSlopeQ2.5))

powerdata%>% 
  mutate(checkSlo = ifelse(widthSlo < .7, 1, 0)) %>% 
  summarise(`proportion below 0.7` = mean(checkSlo),
            `average width`        = mean(widthSlo))
#formula
powerdata <-powerdata%>% 
  mutate(widthformula = abs(formulaQ97.5 - formulaQ2.5))

powerdata%>% 
  mutate(checkformula = ifelse(widthformula < .7, 1, 0)) %>% 
  summarise(`proportion below 0.7` = mean(checkformula),
            `average width`        = mean(widthformula))
