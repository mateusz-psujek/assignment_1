...
update_fg <- function(seed, n){
  df <- 
    update(fg_m,
       newdata = simulate_data(seed = seed, n = n),
       seed = seed, 
       iter = 1000) %>% 
    gather_draws(c(`b_diagnosisasd:visit`,`b_diagnosistd:visit`)) %>%                                                mean_qi(.width = 0.95) %>% 
        rename_with(.cols = everything(), ~ str_remove_all(.x, fixed('.'))) %>%
        add_row
    
    df$variable[[3]] <- 'diff'
    df$value[[3]] <- df$value[[2]] - df$value[[1]]
    df$lower[[3]] <- df$lower[[2]]- df$lower[[1]]
    df$upper[[3]] <- df$upper[[2]] - df$upper[[1]]
    
  df <- df %>% slice(3)
}

power_data <- tibble(N = c(10,150, by = 10)) %>% 
   rowwise %>% 
   mutate(seed = list(seq(1,50, by = 1))) %>% 
   unnest(seed) %>% 
   mutate(dfs = map2(.x = seed, .y = N, .f = ~ update_fg(.x, .y))) %>% 
   unnest(dfs)

  
map(.x = power_data %>% filter(variable == 'diff') %>% group_split(N),
    .f = ~
        ggplot(.x, aes(x = seed %>% reorder(lower), y = value, ymin = lower, ymax = upper)) +
 ...
 ggplot(power_data, aes(x = seed %>% reorder(lower), y = value, ymin = lower, ymax = upper)) +
          geom_hline(yintercept = c(0, 0.2), color = "white") +
          geom_pointrange(fatten = 1/2) +
          labs(title = "Difference of mean slope parameters between the two groups",
...
```{r}
powerdata <-powerdata%>% 
  mutate(ci_width = abs(diff_upper - diff_lower))

powerdata%>% 
  mutate(prec_check = ifelse(ci_width < 0.7, 1, 0)) %>% 
  summarise(`proportion below 0.7` = mean(prec_check),
